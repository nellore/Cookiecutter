...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description='Reproduce published analysis with small data subset or with user defined fastq files.')
	parser.add_argument('-1','--fastq1', help='Left fastq file', required=False, default="../demo/ERR194147_subset_1.fastq")
	parser.add_argument('-2','--fastq2', help='Right fastq file', required=False, default="../demo/ERR194147_subset_2.fastq")
	...
	command = "../src/remove -1 %(fastq1)s -2 %(fastq2)s -o %(output_dir_1a)s --fragments ../data/illumina.dat" % data
	...
	# command = "../src/remove -i %(transc_fastq)s -o %(output_dir_1c)s --fragments ../data/rdna.dat" % data
	# print "Running analysis 1c (rRNA removing from transcripteme data)..."
	# print command
	# os.system(command)

	# command = "../src/extract -1 %(fastq1)s -2 %(fastq2)s -o %(output_dir_1d)s --fragments ../data/mtdna.dat" % data
