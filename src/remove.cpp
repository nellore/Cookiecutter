                         ...
                         Stats & stats, Node * root, std::vector <std::pair<std::string, Node::Type> > const & patterns, int errors)
{
    ...
    while (read.read_seq(reads_f)) {
        ReadType type = check_read(read.seq, read.qual, root, patterns, 0, 0, 0, errors, 0);
        stats.update(type);
        if (type == ReadType::ok) {
            read.write_seq(ok_f);
        }
                         ...
                         Node * root, std::vector <std::pair<std::string, Node::Type> > const & patterns, int errors)
{
    ...
    while (read1.read_seq(reads1_f) && read2.read_seq(reads2_f)) {
        ReadType type1 = check_read(read1.seq, read1.qual, root, patterns, 0, 0, 0, errors, 0);
        ReadType type2 = check_read(read2.seq, read2.qual, root, patterns, 0, 0, 0, errors, 0);
